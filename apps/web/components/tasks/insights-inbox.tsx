...
/** The Insights view of the Tasks inbox: every open (non-dismissed)
 * Insight across every household, grouped by household. This was the whole
 * Tasks page before Task rows existed (D-034); it stays as a second view
 * because an insight is a prompt to consider, not a thing to do, and
 * folding the two together would hide the difference. Dismissing here
 * calls the same server action the household pages use, so it persists. */
export async function InsightsInbox({ q, householdId, section }: { q: string; householdId: string; section: string }) {
  ...
  return (
    <>
      <div className="mb-5 flex flex-wrap items-center gap-2">
        <SearchBox q={q} placeholder="Search insights…" />
      ...
      {groups.length === 0 ? (
        <EmptyState
          title={insights.length === 0 ? "Nothing open" : "No insights match"}
          description={
            insights.length === 0
...
/** A GET form: typing and pressing Enter puts `q` in the URL, alongside
 * whatever FilterSelect has already put there. */
export function SearchBox({ q, placeholder, hidden }: { q: string; placeholder: string; hidden?: Record<string, string> }) {
  return (
    <form className="flex w-60 items-center gap-2 rounded-control border border-rule bg-surface px-2.5 py-2">
      ...
      {hidden && Object.entries(hidden).map(([k, v]) => v && <input key={k} type="hidden" name={k} value={v} />)}
      <input
        type="text"
        name="q"
        defaultValue={q}
